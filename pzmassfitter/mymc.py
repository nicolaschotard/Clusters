...
Here is a quick overview of the class structure:
 ...
 Updater objects provide the mechanism to sample a ParameterSpace. They come in three varieties:
   ...
   emceeUpdater is an interface to the emcee package, and is somewhat different than described below; see its docstring.
...
try:
    ...
    After each call to emceeUpdater, the parameter values are set to those of one of the walkers (a different one each time). The usual backend output should therefore be useful, although arguably it would be better to output the values for all the walkers every N steps. The onStep argument to the Engine could be used for this, or a specialized ParameterSpace/backend combination could be defined.
        """
        def __init__(self, space, nwalkers=None, threads=1, pool=None):
            ...
            self.sampler = emcee.EnsembleSampler(nwalkers, len(space), emcee_lnprobfn, args=[self], threads=threads, pool=pool)
            self.prob = None
            self.rstate = None
            self.pos = emcee.EnsembleSampler.sampleBall([p() for p in space], [p.width for p in space], nwalkers)
            self.nwalkers = nwalkers
        def __call__(self, struct):
            self.structptr = struct
            for self.pos, self.prob, self.rstate in self.sampler.sample(self.pos, self.prob, self.rstate, 1):
                pass
